...
runners.forEach(function (a) {
    return fullName.push(`${a.first_name} ${a.last_name}`)
});
...
// Problem 1: Use two methods to make an array of name tags with first name, last name, and the company listed on them.

let nametag = runners.map(a => `Hello! My name is ${a.first_name} ${a.last_name}, from ${a.company_name}.`);
console.log("Challenge 5, #1 :", nametag);

// Problem 2: Use .filter to make a list of companies that donated over $100, and then print the list of companies sorted by donation amount.

let topDonors = runners.filter(a => a.donation >= 100)
                        .sort((a, b) => b.donation - a.donation);

console.log("Challenge 5, #2: ", topDonors);

// Problem 3: Create a thank you card for donors telling them thank you for their donation amount and listing how much was raised in total.

let thankYous = runners.map(a => `Dear ${a.company_name},
